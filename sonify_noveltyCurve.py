...
def sonify_noveltyCurve(novelty, f_audio, parameter = lambda:0):
    
    if (not hasattr(parameter, 'Fs')):
        parameter.Fs = 22050
    if (not hasattr(parameter, 'featureRate')):
        warn('parameter.featureRate not set! Assuming 100!')
        parameter.featureRate = 100
    
    pos = np.append(novelty, novelty[:,-1]) > np.append(novelty[:,0], novelty)
    neg = np.invert(pos)
    def getIndices(arr, func):
        return np.array([i for (i, val) in enumerate(arr) if func(val)])
    peaks = getIndices(np.multiply(pos[:-1], neg[1:]), lambda x: x == True)

    values = novelty[:,peaks]
    values /= np.max(values)
    peaks = peaks[values[0] > 0.01]
    #peaks = peaks[getIndices(values[0] > 0.01, lambda x: x == True)]
    values = values[:, values[0] > 0.01]
    ...
    click = click * np.power(np.arange(1, 1/len(click) - 0.0001, -1/len(click)), 2)
    #Diverso dall'originale, potrei esportare l'originale e referenziarlo come
    #parte della libreria
    click = resample(click,  2*44100, parameter.Fs)

    out = np.zeros(f_audio.shape[:])
    indexes = np.zeros(peaks.shape[:])
    
    for i in range(0, len(peaks)):
        start = int(np.floor(peaks[i]/parameter.featureRate*parameter.Fs))
        stop = int(start + len(click))
        indexes[i] = (start + stop)/2
        if stop <= len(out):
            out[start:stop] = out[start:stop] + np.multiply(click, values[:,i])
    
    out = np.matrix([f_audio, out])
    return indexes, out